---
name: bazi-ziwei
description: 八字 + 紫微斗数 AI 排盘与综合分析，含“八字 + 车牌选号”娱乐参考原型。当用户提供生辰（阳历/农历日期、时辰、性别）询问八字、紫微、命理、命盘、流年大运，或提供候选车牌希望按八字五行做车牌推荐时使用。基于 Yiqi 算法库 + enrichBazi 补全层精准排盘（杜绝 LLM 自行排盘出错），支持按需独立分析八字 / 独立分析紫微 / 两盘交叉印证 / 车牌评分排序。
---
...
- 询问特定大运/流年的吉凶
- 提供候选车牌并询问"哪个车牌适合我 / 按八字选车牌 / 车牌号码推荐 / 上牌选号"
...
当用户目标是车牌推荐时，跳过命盘分析的 Step 0 决策门，改走本分支。

### 需要收集

```
公历出生日期 (YYYY-MM-DD)
出生时间 (HH:MM；允许 unknown/未知)
性别 (男/女/未知)
候选车牌列表
出生地（可选）
当前省份/城市前缀（可选，例如 粤 或 粤B）
```

若用户没有给候选车牌，先请用户补充候选列表。若出生时间未知，可以继续运行，但必须说明这是低置信度的简化参考。

### 调用本地算法

优先使用可复现 CLI，不要把评分规则写在对话 prompt 里：
...
若未安装 `calculator` 依赖，CLI 会退回低置信度简化参考，并在输出里提示先运行 `cd calculator && npm install`。规则配置在 `config/license_plate_rules.json`，评分逻辑在 `src/license_plate_bazi/`。如果需要更细的工作流说明，读取 `references/license-plate-bazi-workflow.md`。可复制示例见 `examples/license_plate_demo.md`。

### 输出要求

- 按综合分从高到低排序
- 展示每个车牌的 0-100 分、推荐等级、五个分项得分
- 解释八字五行偏好、数字五行、字母五行、重复/连号/谐音/易记性、冲突点
- 末尾必须声明：结果仅作传统文化和娱乐参考，不具备科学预测效力，也不作为实际决策依据

### 禁止事项

- 不要承诺财富、健康、事故、婚姻等结果
- 不要说某个号码“必然带来好运”或“必然不好”
- 不要绕过 `config/license_plate_rules.json` 临时编一套不可复现评分
- 不要在用户缺少候选车牌时凭空生成“最佳车牌”
